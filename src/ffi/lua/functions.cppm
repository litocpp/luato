...
export {
  ...
  inline auto lua_pcall(lua_State *state, int argument_count, int result_count,
                        int error_function) -> int {
    return lua_pcallk(state, argument_count, result_count, error_function, 0,
                      nullptr);
  }
  ...
  inline auto lua_getextraspace(lua_State *state) noexcept -> void * {
    return static_cast<void *>(reinterpret_cast<char *>(state) -
                               LUA_EXTRASPACE);
  }
  ...
  inline void lua_newtable(lua_State *state) { lua_createtable(state, 0, 0); }

  inline void lua_register(lua_State *state, char const *name,
  ...
  inline auto lua_pushliteral(lua_State *state, char const *value)
      -> char const * {
  ...
  inline void luaL_argcheck(lua_State *state, bool condition, int argument,
                            char const *message) {
    if (!condition)
      static_cast<void>(luaL_argerror(state, argument, message));
  }

  inline void luaL_argexpected(lua_State *state, bool condition, int argument,
                               char const *type_name) {
    if (!condition)
      static_cast<void>(luaL_typeerror(state, argument, type_name));
  }
  ...
  inline auto luaL_optstring(lua_State *state, int argument,
                             char const *fallback) -> char const * {
  ...
  inline auto luaL_dostring(lua_State *state, char const *source) -> int {
    return luaL_loadstring(state, source) ||
  ...
  inline auto luaL_opt(lua_State *state, Function function, int argument,
                       Default fallback)
  ...
  inline auto luaL_loadbuffer(lua_State *state, char const *buffer, size_t size,
                              char const *name) -> int {
    return luaL_loadbufferx(state, buffer, size, name, nullptr);
  }

  inline void luaL_pushfail(lua_State *state) { lua_pushnil(state); }
  ...
  inline void luaL_addchar(luaL_Buffer *buffer, char value) {
    if (buffer->n >= buffer->size)
      static_cast<void>(luaL_prepbuffsize(buffer, 1));
